  ...
  Future<Map<String, dynamic>> handleRequest(
    ...
    switch (method) {
      case 'POST':
        ...
        try {
          final model = create_request.CreateInvoiceDefaultRequest(
          ...
          final CreateInvoiceDefaultResponse response =
              await GetIt.I.get<DraftOrderService>().createInvoiceDefault(
          ...
          return {
            "status": "success",
            "message": "Default draft order invoice sent successfully",
          ...
          return {
            "status": "error",
            "message":
                "Failed to send default draft order invoice: ${e.toString()}",
            "timestamp": DateTime.now().toIso8601String(),
          };
        }
      ...
      default:
        return {
          "error":
              "Method $method not supported for Send Default Draft Order Invoice API",
        };
    }
  }
