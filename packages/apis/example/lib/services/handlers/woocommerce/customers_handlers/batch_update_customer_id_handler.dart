...
///*******************************************************************
//************ 📦 BATCH UPDATE CUSTOMER ID HANDLER *****************
///*******************************************************************

class BatchUpdateCustomerIdHandler implements ApiRequestHandler {
  @override
  Future<Map<String, dynamic>> handleRequest(
    ...
    if (method != 'PATCH') {
      return {
        "status": "error",
        "message": "Method $method not supported for Batch Update Customer ID API",
    ...
    try {
      ...
      final response = await GetIt.I<CustomersService>().batchUpdateCustomerId(
      ...
      return {
        "status": "success",
        "message": "Batch update for customer ID completed successfully",
        "data": response,
        "timestamp": DateTime.now().toIso8601String(),
      };
    } catch (e) {
      return {
        "status": "error",
        "message": "Failed to batch update customer ID: ${e.toString()}",
        "timestamp": DateTime.now().toIso8601String(),
      };
    }
  }
  ...
  Map<String, List<ApiField>> get requiredFields => {
        'PATCH': [
          const ApiField(
            name: 'customer_id',
            label: 'Customer ID',
            hint: 'The ID of the customer for batch operations',
            isRequired: false,
          ...
          const ApiField(
            ...
            hint: 'Create operations in format: email:firstName:lastName:role:username',
            isRequired: false,
          ),
          const ApiField(
            name: 'update_operations',
            label: 'Update Operations',
            hint: 'Update operations in format: id:email:firstName:lastName:role:username',
            isRequired: false,
          ),
          const ApiField(
