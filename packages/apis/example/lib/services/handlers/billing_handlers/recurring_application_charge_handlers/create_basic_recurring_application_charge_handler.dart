...
class CreateBasicRecurringApplicationChargeHandler implements ApiRequestHandler {
  @override
    ...
    try {
      if (method == 'POST') {
        ...
        if (charge == null) {
          return {
            "status": "error",
            "message": "Failed to create recurring application charge - no data in response",
            "timestamp": DateTime.now().toIso8601String(),
          };
        }
        
        debugPrint('✅ Successfully created basic recurring application charge with ID: ${charge.id}');
      ...
      debugPrint('❌ Error creating basic recurring application charge: ${e.toString()}');
      return {
        "status": "error",
        "message": "Failed to create basic recurring application charge: ${e.toString()}",
        "apiVersion": ApiNetwork.apiVersion,
        "params": params,
        "timestamp": DateTime.now().toIso8601String(),
      };
